...
# Generate the certs
openssl req -newkey rsa:4096 -x509 -sha256 -days 365 -nodes -out client.crt -keyout client.key -subj "/CN=${SDKMS_MONGO_APP_ID}"
...
curl -s -X PATCH -H "Authorization:Bearer $SDKMS_API_KEY" -d "{\"credential\":{\"certificate\":\"$CERT_INFO\"}}" https://${SDKMS_API_ENDPOINT}/sys/v1/apps/${SDKMS_MONGO_APP_ID}
